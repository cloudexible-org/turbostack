...
* **Page Objects Pattern:** Whenever a new page is created in `apps/app` or `apps/www`, or a significant UI component is added to `packages/ui`, you must:
    1.  Update or create the corresponding file in that app's suite: `apps/e2e-app/page-objects/` or `apps/e2e-www/page-objects/`.
    2.  Ensure selectors are resilient (prefer data-attributes like `data-testid` over CSS classes).
* **E2E / Integration Tests:** One Playwright package per app. `apps/e2e-app` drives the Vite app (`apps/app`) with two projects: `app` (static chrome, `specs/app/`) and `app-convex` (against a real, seeded Convex local backend, `specs/app-convex/`). `apps/e2e-www` drives the Next.js marketing site (`apps/www`), `specs/`. Put a spec in the suite of the app it asserts against. Run `pnpm test:e2e:app`, `pnpm test:e2e:www`, or `pnpm test:e2e` for both. Shared harness code (free ports, the run lock) lives in `packages/e2e-kit`. Ensure that new features are accompanied by a Playwright test script utilizing the updated page objects.
* **Convex-backed specs:** Anything asserting on backend data belongs in `apps/e2e-app/specs/app-convex/`, seeded from `packages/api/convex/seed/e2e/fixture.ts`. The suite talks only to a **local, anonymous** Convex deployment, booted per run on ports the OS hands it — never hard-code 3210 or read the port from `.convex/local/default/config.json`, and read `docs/e2e-architecture.md` §1a and §1c before touching that harness: seeding wipes the database, and a wrong port can wipe a *different project's or worktree's*.
