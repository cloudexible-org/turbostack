...
* **E2E / Integration Tests:** The `apps/e2e` directory holds one Playwright project per app: `app` drives the Vite app (`apps/app`) on `http://127.0.0.1:5173`, `www` drives the Next.js marketing site (`apps/www`) on `http://127.0.0.1:3100`, and `app-convex` drives `apps/app` against a real, seeded Convex local backend. Specs live in `specs/<app>/` and page objects in `page-objects/<app>/`; each project pins its own `baseURL`, so put a spec in the directory of the app it asserts against. Ensure that new features are accompanied by a Playwright test script utilizing the updated page objects.
* **Convex-backed specs:** Anything asserting on backend data belongs in `specs/app-convex/`, seeded from `packages/api/convex/seed/e2e/fixture.ts`. The suite talks only to a **local, anonymous** Convex deployment, whose port is read from `.convex/local/default/config.json` — never hard-code 3210, and read `docs/e2e-architecture.md` §1a before touching that harness: seeding wipes the database, and a hard-coded port can wipe a *different project's*.
* **The suite is independent of `pnpm dev`:** both can run at once. It allocates its own free ports, builds Next into its own `distDir`, and drives its own local Convex backend, so it never touches your dev servers or your cloud deployment. Keep it that way — do not reintroduce a fixed port, a shared `distDir`, or `reuseExistingServer: true` for an app server.
